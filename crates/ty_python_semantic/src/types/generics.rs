...
impl<'db, 'c> SpecializationBuilder<'db, 'c> {
    ...
    /// Returns an error if an actual-intersection constraint set has no valid specialization.
    fn validate_intersection_constraint_set(
        &self,
        set: ConstraintSet<'db, 'c>,
    ) -> Result<(), ConstraintSetInferenceError<'db>> {
        let mut typevar_errors = Vec::new();
        match set.solutions_with(
            self.db,
            self.constraints,
            self.inferable,
            |_variance, path_bound| {
                ...
                if solution.is_err()
                    && let Some(error) = Self::specialization_error_from_failed_bounds(
                        self.db,
                        self.constraints,
                        path_bound,
                    )
            ...
            Solutions::Unsatisfiable => Err(Self::combine_intersection_specialization_errors(
                self.db,
                &typevar_errors,
            )
            .map_or(
            ...
            Solutions::Unconstrained | Solutions::Constrained(_) => Ok(()),
        }
    }

    /// Preserve one diagnostic for the whole failed intersection when every path rejected the same
    /// type variable. Lower-bound evidence meets across positive intersection elements; upper-bound
    /// evidence joins.
    fn combine_intersection_specialization_errors(
        db: &'db dyn Db,
        errors: &[(SpecializationError<'db>, bool, bool)],
    ) -> Option<SpecializationError<'db>> {
        let first = errors.first()?.0.clone();
        let same_typevar = errors
            .iter()
            .all(|(error, _, _)| error.bound_typevar() == first.bound_typevar());
        let same_kind = errors.iter().all(|(error, _, _)| {
            matches!(
                (&first, error),
                (
        ...
        if errors.len() < 2 || !same_typevar || !same_kind {
            return Some(first);
        }

        let has_only_lower_bounds = errors
            .iter()
            .all(|(_, has_lower, has_upper)| *has_lower && !has_upper);
        let has_only_upper_bounds = errors
            .iter()
            .all(|(_, has_lower, has_upper)| !has_lower && *has_upper);
        let argument = if has_only_lower_bounds {
            IntersectionType::from_elements(
                db,
                errors.iter().map(|(error, _, _)| error.argument_type()),
            )
        } else if has_only_upper_bounds {
            UnionType::from_elements(db, errors.iter().map(|(error, _, _)| error.argument_type()))
        } else {
            return Some(first);
        };
        Some(match first {
        ...
        let solutions = match set.solutions_with(
            ...
            |_variance, path_bound| {
                ...
                if solution.is_err() && first_typevar_error.is_none() {
                    first_typevar_error = Self::specialization_error_from_failed_bounds(
                        self.db,
                        self.constraints,
                        path_bound,
                    );
    ...
    fn specialization_error_from_failed_bounds(
        db: &'db dyn Db,
        constraint_set_builder: &ConstraintSetBuilder<'db>,
        ...
        match bound_typevar.typevar(db).bound_or_constraints(db)? {
            ...
            TypeVarBoundOrConstraints::Constraints(constraints) => {
                let is_possibly_assignable = |source: Type<'db>, target: Type<'db>| {
                    ...
                    !constraint_set_builder
                        .load(db, &when)
                        .is_never_satisfied(db)
                };
                let declared_constraints = constraints.elements(db);

                // If either side of the path independently rules out every declared constraint,
                // we can report that mismatch. Otherwise, the combination of lower and upper
                // bounds may be what invalidated the path.
    ...
    fn infer_map_impl(
        ...
        match (formal, actual) {
            ...
            (formal, actual @ Type::Intersection(_)) => {
                ...
                match self.validate_intersection_constraint_set(when) {
                    Ok(()) => {
