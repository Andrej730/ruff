...
fn enum_names_type<'db>(context: &InferContext<'db, '_>) -> Type<'db> {
    let db = context.db();
    let python_version = context.python_version();
    let str_type = KnownClass::Str.to_instance_with_version(db, python_version);
    let iterable_str =
        KnownClass::Iterable.to_specialized_instance_with_version(db, python_version, &[str_type]);
    let iterable_object = KnownClass::Iterable.to_specialized_instance_with_version(
        db,
        python_version,
        &[Type::object()],
    );
    let iterable_iterable_object = KnownClass::Iterable.to_specialized_instance_with_version(
        db,
        python_version,
        &[iterable_object],
    );
    let mapping_str_object = KnownClass::Mapping.to_specialized_instance_with_version(
        db,
        python_version,
        &[
...
fn first_enum_auto_value<'db>(
    ...
    match base_class {
        ...
        _ => match start {
            ...
            EnumStart::DynamicInt => {
                KnownClass::Int.to_instance_with_version(db, context.python_version())
            }
        },
    }
}
    ...
    match base_class {
        ...
        _ => {
            let Some(last) = last_int_value else {
                return KnownClass::Int.to_instance_with_version(db, context.python_version());
            };
            match base_class {
...
impl<'db> TypeInferenceBuilder<'db, '_> {
    pub(crate) fn infer_enum_call_expression(
        ...
        let Some(names_arg) = names_arg else {
            ...
            return Some(base_class.to_instance_with_version(db, python_version));
        };
        ...
        let Some(name) = self.infer_enum_name_argument(name_arg, base_class) else {
            return SubclassOfType::try_from_type(
                db,
                base_class.to_class_literal_with_version(db, self.python_version()),
            );
        };
    ...
    fn infer_enum_mixin_argument(
        ...
        if let Some(class_lit) = ty.as_class_literal() {
            ...
            let Some(enum_base) = base_class
                .to_class_literal_with_version(db, self.python_version())
                .to_class_type(db)
            else {
                return (Some(ty), true);
            };
    ...
    fn parse_enum_members_arg(
        ...
        if ty.is_dynamic() || ty.is_assignable_to(db, enum_names_type(&self.context)) {
    ...
    fn report_invalid_enum_names_argument(
        ...
        if let Some(builder) = self.context.report_lint(&INVALID_ARGUMENT_TYPE, names_arg) {
            ...
            diagnostic.set_primary_message(format_args!(
                "Expected `{}`, found `{}`",
                enum_names_type(&self.context).display(db),
                names_ty.display(db),
            ));
        }
    }
