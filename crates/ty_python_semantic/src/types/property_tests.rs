...
mod stable {
    ...
    type_property_test!(
        ...
        forall types t. t.is_assignable_to(db, KnownClass::Iterable.to_specialized_instance(db, &[Type::object()])) => t.try_iterate(db, crate::Program::get(db).python_version(db)).is_ok()
    );
