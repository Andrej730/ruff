...
## Non-determinism with gradual types

Overload evaluation is order independent. A gradual constraint inferred from an earlier overload
candidate should not influence the result of a later candidate:
...
def result[T](children: list[Item[T]]):
    # error: [missing-type-argument]
    def key(item: Item) -> int:
        return 0

    ordered_result = ordered(children, key=key)
    reveal_type(ordered_result)  # revealed: list[Item[T@result]]

    reversed_result = ordered_reversed(children, key=key)
    reveal_type(reversed_result)  # revealed: list[Item[T@result]]
```
