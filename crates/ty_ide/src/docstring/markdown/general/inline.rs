...
use crate::docstring::document::syntax::BacktickRuns;
...
fn find_link(input: &str) -> Option<(usize, Candidate<'_>)> {
    let mut runs = BacktickRuns::new(input);
...
fn parse_candidate(input: &str) -> Option<Candidate<'_>> {
    ...
    let Some(closing) = runs.next() else {
    ...
    let span = runs.span(opening, closing)?;
    if !span.is_single() {
        return None;
    }
