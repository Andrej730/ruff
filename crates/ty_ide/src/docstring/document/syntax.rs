...
/// ```
pub(in crate::docstring) struct BacktickFragments<'a> {
...
impl<'a> BacktickFragments<'a> {
    ...
    pub(in crate::docstring) fn new(source: &'a str) -> Self {
        Self {
            runs: BacktickRuns::new(source),
    ...
    fn take_remaining_text(&mut self) -> Option<BacktickFragment<'a>> {
        let source_end = TextSize::of(self.runs.source);
        let remaining = TextRange::new(self.last_fragment_end, source_end);
        self.last_fragment_end = source_end;
        (!remaining.is_empty()).then(|| BacktickFragment::Text(&self.runs.source[remaining]))
    }
}
...
impl<'a> Iterator for BacktickFragments<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if let Some(span) = self.pending_span.take() {
            ...
            return Some(BacktickFragment::Span(span));
        }

        if self.last_fragment_end == TextSize::of(self.runs.source) {
            return None;
        }

        loop {
            let Some(opening) = self.runs.next() else {
            ...
            let Some(span) = self.runs.eat_span(opening) else {
                return self.take_remaining_text();
            };
            ...
            if self.last_fragment_end < span.start() {
                ...
                return Some(BacktickFragment::Text(&self.runs.source[preceding_text]));
            }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::docstring) enum BacktickFragment<'a> {
...
/// ```
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::docstring) struct BacktickSpan<'a> {
...
impl<'a> BacktickSpan<'a> {
    ...
    pub(in crate::docstring) fn is_single(self) -> bool {
        self.delimiter_len == TextSize::new(1)
    }

    /// Returns the source between the opening and closing runs.
    pub(in crate::docstring) fn content(self) -> &'a str {
        self.content
    }
}
...
/// next()                  Some(BacktickRun("``"))
/// as_str()                "code`` suffix"
/// eat_span(opening)       Some(BacktickSpan("``code``"))
/// as_str()                " suffix"
/// ```
#[derive(Clone)]
pub(in crate::docstring) struct BacktickRuns<'a> {
...
impl<'a> BacktickRuns<'a> {
    ...
    pub(in crate::docstring) fn starts_at(offset: TextSize, source: &'a str) -> Self {
    ...
    pub(in crate::docstring) fn as_str(&self) -> &'a str {
        self.cursor.as_str()
    }
    ...
    pub(in crate::docstring) fn eat_span(
        &mut self,
        opening: BacktickRun,
    ...
    pub(in crate::docstring) fn span(
        &self,
        opening: BacktickRun,
        closing: BacktickRun,
    ) -> Option<BacktickSpan<'a>> {
        if opening.end() > closing.start() || opening.range.len() != closing.range.len() {
            return None;
        }
...
impl Iterator for BacktickRuns<'_> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let escaped = !self.source[..start.to_usize()]
            .bytes()
            .rev()
            .take_while(|byte| *byte == b'\\')
            .count()
...
/// One consecutive run of backticks found by [`BacktickRuns`].
///
/// For example:
///
/// ```text
...
/// ```
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::docstring) struct BacktickRun {
...
impl BacktickRun {
    ...
    pub(in crate::docstring) fn is_escaped(self) -> bool {
        self.escaped
    }
}
...
mod tests {
    use super::{
        BacktickFragment, BacktickFragments, BacktickRuns, TextSize, is_markdown_code_span,
        split_once_at_top_level_colon, split_trailing_parenthetical,
    };

    #[test]
    fn scans_backtick_runs_and_spans() {
        let mut runs = BacktickRuns::starts_at(TextSize::new(7), "prefix ``code`` suffix");
        let opening = runs.next().expect("an opening backtick run");

        assert!(!opening.is_single());
        assert!(!opening.is_escaped());
        assert_eq!(runs.as_str(), "code`` suffix");

        let span = runs.eat_span(opening).expect("a matching backtick run");
        assert!(!span.is_single());
        assert_eq!(span.content(), "code");
        assert_eq!(runs.into_cursor().as_str(), " suffix");
    }
